...
export const UpdateAdmin = async (req, res) => {
  try {
    const { id, username, email, phone, password, role, restriction } = req.body;
  ...
  try {
    ...
    if (req.file) {
      if (existing.profileImage) {
        deleteFile(existing.profileImage, "profiles");
      }
    ...
    const updated = await AdminModel.findById(id).select("-password");
